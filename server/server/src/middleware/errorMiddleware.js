...
const errorMiddleware = (err, req, res, next) => {
  ...
  if (err.code === 11000) error = new ApiError(409, `Duplicate value: ${Object.keys(err.keyValue || {}).join(', ')}`);
  if (err.name === 'ValidationError') error = new ApiError(400, 'Validation failed.', Object.values(err.errors).map(e => e.message));
