...
export default function Education() {
  ...
  const load = () =>
    api.get("/educations/admin").then((r) => setItems(r.data.data));
  useEffect(() => {
    load();
  }, []);
  const save = async (e) => {
    e.preventDefault();
    try {
      if (editing) await api.put(`${"/educations"}/${editing}`, form);
      else await api.post("/educations", form);
      toast.success("Saved");
      setForm({ institution: "", degree: "", field: "", result: "" });
      setEditing(null);
      load();
    } catch (err) {
  ...
  const edit = (x) => {
    setEditing(x._id);
    setForm({
      ...{ institution: "", degree: "", field: "", result: "" },
      ...x,
    });
  };
  const del = async (id) => {
  ...
  return (
    <div>
      <h1 className="text-3xl font-black mb-6">Education</h1>
